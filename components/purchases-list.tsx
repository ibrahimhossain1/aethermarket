...
import { useMockPurchases, MockPurchaseItem } from "@/lib/store/useMockPurchases"
...
export default function PurchasesList({ initialPurchases, isDbOffline }: PurchasesListProps) {
  const searchParams = useSearchParams()
  const { data: session } = useSession()
  const mockStore = useMockPurchases()
  ...
  // 1. Process URL redirects for simulator checkouts
  React.useEffect(() => {
    ...
    if (success === "true" && mock === "true" && productId) {
      // Look up product specs from mock data to save full context
      ...
      if (product) {
        mockStore.addPurchase({
          ...
          productId,
          stripePaymentIntentId: `mock_stripe_sim_${Math.random().toString(36).substring(2, 10)}`,
  ...
  const displayPurchases = React.useMemo(() => {
    if (!mounted) return initialPurchases

    if (isDbOffline) {
      const buyerId = session?.user?.id || ""
      return mockStore.purchases.filter((p) => p.buyerId === buyerId)
    }

    return initialPurchases
  }, [mounted, initialPurchases, isDbOffline, mockStore.purchases, session])

  if (!mounted) {
    return (
      <div className="flex flex-col gap-4 animate-pulse">
        <div className="h-20 bg-zinc-900/40 rounded-2xl border border-zinc-800"></div>
        <div className="h-20 bg-zinc-900/40 rounded-2xl border border-zinc-800"></div>
      </div>
    )
  }
